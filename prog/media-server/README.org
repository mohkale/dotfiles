...
**** Bypassing ISPs CG-NAT issues with IPv6
     ...
     Luckily ddns-updater supports IPv6 as well, but docker doesn't (very well). To
     update DNS records with IPv6 you must first [[https://docs.docker.com/config/daemon/ipv6/][enable IPv6 support in your docker
     daemon]]. To test if IPv6 is working as at the docker daemon level you can use
     the curl docker container like so:
     #+begin_src shell
       docker run --rm -it curlimages/curl -6 icanhazip.com
     #+end_src

     After this you can simply set =ip_version= to /ipv6/ instead of /ipv4/ in your
     ddns-updater configuration file and restart the service. At least it would be
     nice if you could 😢. I could not for some reason docker-compose seemed to have
     it's own issues with IPv6 independent of the docker daemon. I ended up
     translating my previous docker-compose configuration into a docker command and
     that at least seems to work. Try testing this and swapping the command in your
     service file to use this instead.

     #+begin_src shell
       # To start the service
       docker run --rm -d --name ms-ddns-updater --publish 8000:8000 --volume /etc/media-server/config/ddns-updater:/updater/data qmcgaw/ddns-updater
       # To stop the service
       docker stop ms-ddns-updater
     #+end_src
