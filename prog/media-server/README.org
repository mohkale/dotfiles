...
#+HTML:   <a style="margin-left: 4px;" href="https://github.com/AnalogJ/scrutiny"><img src="https://img.shields.io/badge/scrutiny-latest:latest-blue?logo=googlecloudstorage&style=for-the-badge"/></a>
#+HTML: </div>
...
** Prowlarr                                                         :tracker:
   ...
   *Note*: You may want to add a download client entry for your bittorrent daemon in
...
** qBittorrent                                                   :bittorrent:
   ...
   By default the username for the qBittorrent Docker image is admin and a unique
   password will be logged when the application starts up.
   ...
   Completed torrents will be moved by the [[file:qbittorrent/scripts/torrent-done][torrent-done]] script directly.
...
** Scrutiny                                                              :ui:
   Scrutiny is a nice web UI for monitoring hard-drive health and S.M.A.R.T. reports.
   After enabling you'll need to mount the underlying disk devices for any disks you
   want scrutiny to query the reports for into the scrutiny container. Unfortunately
   it is not possible to do this through device ids or a UUID so you'll have to rely
   on the randomly assigned /dev/sdx naming schemes.

   #+CAPTION: Sample override for [[file:9992-optional.scrutiny.override.yml]] passing forward any disk devices.
   #+begin_src yaml
     ---
     services:
       scrutiny:
         devices:
           - /dev/sda # /dev/disk/by-id/wwn-0x5000c500db51abc5
           - /dev/sdc # /dev/disk/by-id/wwn-0x5000c500db51def9
   #+end_src
...
** Notifications from QBittorrent
   QBittorrent is configured to notify the logged in user when a torrent completes
   ...
   D-Bus runtime directory into the qBittorrent service.
...
*** Port Forwarding
    ...
    *Note*: If you do not use the VPN support documented here then port forwarding
    should work automatically. The QBittorrent container will automatically reuse
    the network of the host container for the QBittorrent peer ports.
    ...
    =QBITTORRENT_PEER_PORT= environment variables to your peer port. You must also
    ...
    port forwarding you wouldn't traffic from the default service peer port to reach
    your host).
    ...
    #+begin_src yaml
      ...
      services:
        gluetun:
          environment:
            FIREWALL_VPN_INPUT_PORTS: $QBITTORRENT_PEER_PORT
          ports:
...
**** Static IP Routing
     ...
     | A           | *.media.site.com | 27.241.155.98                           |
     | AAAA        | media.site.com   | 2714:d863:fdce:8ceb:31b0:a023:d197:24e6 |
     | AAAA        | *.media.site.com | 2714:d863:fdce:8ceb:31b0:a023:d197:24e6 |
...
**** Bypassing ISPs CG-NAT issues with IPv6
     CG-NAT is a technology where ISPs while combine multiple IPv4 addresses for
     different customers into a single IPv4 addresses. This means the ISP functions
     like a light VPN and multiple users may be using the same IPv4 address as you.
     Unfortunately it also means your ISP won't forward IPv4 packets for externally
     initiated connections to your router because it can't deterministically know
     which of the multiple routers broadcasting the same IPv4 address the packet is
     meant for.

     This shouldn't have a big impact on most services but will almost certainly
     break DDNS support. The only way to bypass this is to have a static IP
     provisioned (at an extra fee of course 👿) or try registering IPv6 DNS records
     instead and hope CG-NAT is not enabled for it. As of writing (25-07-2024) CG-NAT
     is not trivially (read cheap) to setup for ISPs so enabling IPv6 is not a
     priority for most and likely won't be for a long time.

     Luckily ddns-updater supports IPv6 as well, but docker doesn't (very well). At
     this point I'd suggest moving the ddns-updater configuration to your primary
     media-server host and running it through a native install instead of docker.
     This would mean your home network is inaccessible if your ISP moves your IPv6
     address :cry:. I'd also suggest setting some kind of raspberry pi SSH host for
     keeping things accessible remotely even when your primary media-server is
     offline.
...
*** Banning Bad Actors with Fail2Ban
    ...
    While it is possible to use fail2ban through docker, I've had issues getting
    iptable chains set in container to sync back to the host operating system. It
    would be simpler to install and setup locally outside of docker and point
    configurations to files mounted into the service container.

    Here's a sample [[file:$XDG_CONFIG_HOME/media-server/fail2ban/jail.local][jail.local]] file.
    #+begin_src conf
      [DEFAULT]
...
** Bypassing cloudflare protection for certain trackers/indexers
   ...
   prowlarr untenable. You can bypass this using a service called byparr. To
...
** Treating Jellyfin as a DLNA/uPnP Server

   DLNA/uPnP is a system that allows supporting devices on the same network to stream
   content between each other. Where possible a native client app is preferred but
   when this isn't possible uPnP is a nice generally applicable standard for
   broadcasting content.

   To enable you should set the dlna profile. This will run jellyfin in the host OS
   network instead of through a bridge networked container. Next you need to navigate
   to jellyfin admin settings, install the DLNA plugin. You may also have to unblock
   uPnP/DLNA at your router level. Many routers disable this feature for protection.
