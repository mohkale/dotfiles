...
class NordVPNSegment(StatusMiscSegment):
    ...
    def render(self):
        ...
        if not which("nordvpn"):
            logging.debug(
                "Skipping segment=%s because nordvpn is not installed.",
                self.name,
            )
            return None
        proc = subprocess.run(
        ...
        if proc.returncode != 0:
            logging.warning(
                "Warning failed to run nordvpn process for segment=%s",
                self.name,
            )
            return None
        ip_address = ""
        ...
        return (
            self._style(
                self.icon,
                self.active_style if ip_address else self.inactive_style,
            )
            + ip_address
        )
