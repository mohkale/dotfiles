...
class DockerSegment(StatusMiscSegment):
    ...
    def render(self) -> str:
        if not which("docker"):
            logging.debug(
                "Skipping segment=%s because docker is not installed.",
                self.name,
            )
            return None
        proc = subprocess.run(
        ...
        if proc.returncode != 0:
            logging.debug(
                "Warning failed to run docker process for segment=%s",
                self.name,
            )
            return None
        ...
        for status in self.statuses:
            ...
            count_strings.append(
                self._style(
                    self.format.format(count), self.styles.get(status, "")
                )
            )
            status_total += count
        if count_strings:
        ...
        if status_total != count_total:
            if count_strings:
                result += "/"
            result += self._style(
                self.format.format(count_total), self.total_style
            )
        return result
